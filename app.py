...
- 商品名・固有名詞（例：ダイレクトウォーターなど）は正確にそのまま記載。絶対に省略・変更しない
- 行数制限なし。内容を一切省略しない。元の議事録に書かれていることは全て漏れなく記載する
- 特に[TODO]は元の議事録のアクションアイテムを一つも省略せず全て記載すること
...
[TODO]
・担当者：タナカ　内容：ダイレクトウォーター提案書作成　期限：来週金曜
・担当者：スズキ　内容：新規顧客プレゼン資料作成　期限：今月末
